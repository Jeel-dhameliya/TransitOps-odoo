...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-slate-900 to-slate-850 text-white transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 border-r border-slate-800 shadow-xl ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between h-16 px-6 border-b border-slate-800/60 bg-slate-900/50">
          <span className="text-2xl font-bold text-white tracking-tight flex items-center gap-2">
            <img src="/favicon.svg" alt="TransitOps Logo" className="w-8 h-8 drop-shadow-md" />
            <span>Transit<span className="text-blue-400 font-normal">Ops</span></span>
          </span>
