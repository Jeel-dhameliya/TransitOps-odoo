...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate API call for hackathon purposes
    try {
        ...
        <div className="w-full max-w-[440px] relative z-10">
          ...
          {error && (
            <div className="absolute -top-4 -right-2 lg:-right-12 lg:-top-16 w-72 bg-red-50/90 backdrop-blur-md border border-red-200 rounded-2xl p-4 shadow-lg z-20 animate-fade-in text-sm text-red-600 font-medium">
              <div className="text-xs text-red-500 font-bold uppercase tracking-wider mb-1">Error</div>
          ...
          <div className="bg-white/80 backdrop-blur-2xl border border-white shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] rounded-3xl p-8 sm:p-10 relative overflow-hidden transition-all duration-500 hover:shadow-[0_20px_60px_-15px_rgba(59,130,246,0.2)]">
            ...
            {success ? (
              <div className="text-center animate-fade-in py-4">
                ...
                <h2 className="text-2xl font-extrabold text-slate-800 mb-2 tracking-tight">Check your email</h2>
                <p className="text-slate-500 text-sm mb-8 leading-relaxed">
                  We've sent password reset instructions to <strong>{email}</strong>.
                </p>
                <Link 
                  to="/login"
                  className="w-full flex justify-center py-4 px-4 border border-transparent rounded-xl shadow-md text-sm font-bold text-white bg-slate-800 hover:bg-slate-900 transition-all"
                >
                  Return to Login
                </Link>
              </div>
              ...
              <>
                <div className="mb-8">
                  <Link to="/login" className="text-sm font-semibold text-slate-400 hover:text-blue-600 flex items-center gap-2 mb-6 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                    Back to login
                  </Link>
                  <h2 className="text-3xl font-extrabold text-slate-800 mb-2 tracking-tight">Reset password</h2>
                  <p className="text-slate-500 text-sm font-medium leading-relaxed">Enter the email address associated with your account and we'll send you a link to reset your password.</p>
                </div>
